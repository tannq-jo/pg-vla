...
class SpatialVLACausalLMOutputWithPast(ModelOutput):
    ...
    raw_self_attentions: Optional[Tuple[torch.FloatTensor]] = None
    raw_cross_attentions: Optional[Tuple[torch.FloatTensor]] = None
...
class SpatialVLAForConditionalGeneration(SpatialVLAPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        return_attentions: Optional[bool] = True
        ...
        if return_attentions:
            if hasattr(outputs, "attentions"):
                raw_self_attentions = [attn.detach() for attn in outputs.attentions]
            
            if hasattr(outputs, "cross_attentions"):
                raw_cross_attentions = [attn.detach() for attn in outputs.cross_attentions]
    ...
    def process_attention_matrices(
        self,
        model_outputs: SpatialVLACausalLMOutputWithPast,
        include_cross_attentions: bool = True,
        head_fusion_method: str = "mean",
    ):
        attention_maps = {}

        if model_outputs.raw_self_attentions is not None:
            self_attentions = torch.stack(model_outputs.raw_self_attentions) # [bsz, num_heads, seq_len, seq_len]
            self_attentions = self.fuse_attention_heads(self_attentions, head_fusion_method, 2)
            self_attentions = self_attentions.permute(1, 0, 2, 3)
            attention_maps["self_attentions"] = self_attentions

        if include_cross_attentions and model_outputs.raw_cross_attentions is not None:
            cross_attentions = torch.stack(model_outputs.raw_coss_attentions)
            cross_attentions = self.fuse_attention_heads(cross_attentions, head_fusion_method, 2)
            cross_attentions = cross_attentions.permute(1, 0, 2, 3)
            attention_maps["cross_attentions"] = cross_attentions

        return attention_maps
        
    def fuse_attention_heads(
        self,
        attention_matrix: torch.FloatTensor,
        head_fusion_method: Optional[str] = "mean",
        dim: int = 1,
    ) -> torch.FloatTensor:
        """
            Input:
                attention_matrix: [bsz, num_heads, seq_len, seq_len]
                fusion_type: str,
                dim: int
            Output:
                fused_attention: [bsz, seq_len, seq_len]
        """
        if head_fusion_method == "mean":
            ...
            raise ValueError(f"Invalid head fusion method: {head_fusion_method}. Must be one of [\"mean\", \"max\", \"min\"]")
    ...
    def predict_action_with_attention(
        ...
        **kwargs: str
        ...
        kwargs["output_attentions"] = return_attentions
        ...
        if return_attentions:
            ...
            combined_attention = self.combine_attention_matrices(generation_outputs.attentions)
            attention_matrices = self.combine_attention_heads(combined_attention, head_fusion_method, dim=2)
        ...
        return generation_outputs[:, input_len:], attention_data

    def combine_attention_matrices(self, attention_outputs: list):
        """
            Input:
                attention_outputs: [[(batch_size, num_heads, seq_len, seq_len), ...], ...]
        """

        num_tokens = len(attention_outputs)
        num_layers = len(attention_outputs[0])
        batch_size, num_heads, _ , init_seq_len = attention_outputs[0][0].shape
        last_seq_len = init_seq_len + num_tokens - 1

        combined_attention_matrices = torch.zeros(num_layers, batch_size, num_heads, last_seq_len, last_seq_len)
        combined_attention_matrices[..., :init_seq_len, :init_seq_len] = attention_outputs[0][0]

        for i in range(num_tokens):
            for j in range(num_layers):
                if i == 0:
                    combined_attention_matrices[j, :, :, :init_seq_len, :init_seq_len] = attention_outputs[i][j]
                else:
                    current_attention = attention_outputs[i][j][:, :, 0, :]
                    current_pos = current_attention.shape[-1]
                    combined_attention_matrices[j, :, :, current_pos - 1, :current_pos] = current_attention
        
        # (batch_size, num_layers, num_heads, seq_len, seq_len)
        combined_attention_matrices = combined_attention_matrices.permute(1, 0, 2, 3, 4)
        
        return combined_attention_matrices
