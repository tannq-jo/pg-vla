...
class OpenVLAServer:
    ...
    def run(self, host: str = "0.0.0.0", port: int = 8854) -> None:
        self.app = FastAPI()
...
class DeployConfig:
    ...
    port: int = 8854 # Host Port

    # fmt: on


@draccus.wrap()
def deploy(cfg: DeployConfig, *args, **kwargs) -> None:
    server = OpenVLAServer(cfg.openvla_path)
    server.run(cfg.host, port=cfg.port)


if __name__ == "__main__":
    # Cretea an argparse where I can input the port
    args = argparse.ArgumentParser()
    args.add_argument("--port", type=int, default=8854)
    args = args.parse_args()
    deploy(DeployConfig(port=args.port))
